...
<section id="s_edit" class="content">
    <form id="f_upload" enctype="multipart/form-data" method="post">
        ...
        <!--TODO: 在这里补充 html 元素，使 file_upload 上传后若为音乐文件，则可以直接播放done-->
        <audio controls="controls" autoplay="autoplay" id="audiomusic"></audio>
        <input type="file" name="file_upload" onchange="get_URL(this);" id="file_upload">

        <table>
            <tr><td>Title: <input type="text" name="titleinput"></td><td>Artist: <input type="text"></td></tr>
            <tr><td colspan="2"><textarea name="edit_lyric" id="edit_lyric"></textarea></td></tr>
            <tr><td><input type="button" value="插入时间标签" onclick="insertTag(1);"></td><td><input type="button" value="替换时间标签" onclick="insertTag(2);"></td></tr>
            <tr><td colspan="2" id="td_submit"><input type="submit" value="Submit" name="submit"></td></tr>
        </table>
    </form>
</section>
<?php
if($_POST["submit"]){
    $_FILES["file_upload"]["name"] = $_POST["titleinput"].".mp3";
    move_uploaded_file($_FILES["file_upload"]["tmp_name"],"songs/mp3/".$_FILES["file_upload"]["name"]);
    file_put_contents('songs/lrc/'.$_POST["titleinput"].'.lrc',$_POST["edit_lyric"]);
}
    ...
    function get_URL(obj) {
        ...
        xmlhttp.onreadystatechange = function (ev) {
            if (xmlhttp.readyState==4 && xmlhttp.status==200) {
                document.getElementById('audiomusic').innerHTML=xmlhttp.responseText;
            }
        }
        xmlhttp.open("GET","getURL.php?url="+window.URL.createObjectURL(obj.files[0]),true)
        xmlhttp.send();
    }

    //获得插入时间
    function getInsertTime() {
        var seconds = document.getElementById("audiomusic").currentTime;
        insertMin = parseInt(parseInt(seconds)/60);
        insertSec = (parseInt(seconds)%60);
        insertmSce = parseInt((parseFloat(seconds)-parseInt(seconds))*60*100/100);
        return "["+insertMin+":"+insertSec+":"+insertmSce+"]";
    }

    function insertTag(index) {
        var edit_lyric = document.getElementById("edit_lyric");
        var currentTotalNum = edit_lyric.selectionStart;
        var currentRowNum = 0;
        for(var i=currentTotalNum-1;i>=0;i--){
            if(edit_lyric.value.charAt(i)==='\n'){
                break;
            }else{
                currentRowNum++;
            }
        }
        var timeFlag = getInsertTime();
        var n = edit_lyric.selectionStart - currentRowNum;

        if(index == 1){//插入新的
            var insertedContent = "";
            var i = 0;
            if(n>0) {
                var tmp = edit_lyric.value.substring(i, i + n);
                insertedContent += tmp + timeFlag;
                i += n;
                tmp = edit_lyric.value.substring(i, i + n);
                insertedContent += tmp;
            }
            else{
                insertedContent=timeFlag+edit_lyric.value;
            }

            edit_lyric.value = insertedContent;
        }

        if(index == 2){//替换旧的
            var pre = edit_lyric.value.substring(n,edit_lyric.value.length);
            var newtext =  pre.replace( /\[.*]/,timeFlag);//
            edit_lyric.value = edit_lyric.value.replace(pre,newtext);
        }
    }

function chooseMusic() {
    var songname = document.getElementById("select").value;
    var fso = new ActiveXObject("Scripting.FileSystemObject");
    var f = fso.OpenTextFile("songs/lrc/"+songname+".lrc",1);
    alert(f.ReadAll())
    document.getElementById("lyric").value = f.ReadAll();
    f.close();
}
